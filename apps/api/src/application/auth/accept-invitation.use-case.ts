...
export class AcceptInvitationUseCase {
  ...
  async execute(input: AcceptInvitationInput): Promise<{ token: string; user: User }> {
    ...
    const user: User = { id: this.ids.next(), email: inv.email, firstName: input.firstName, lastName: input.lastName, role: inv.role, organizationId: inv.organizationId, createdAt: now, emailVerifiedAt: now };
