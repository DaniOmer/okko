...
describe('DeleteZoneUseCase', () => {
  it('supprime une zone libre', async () => {
    ...
    const uc = new DeleteZoneUseCase(zones, links, presences, audit() as any, clock);
    await uc.execute({ id: 'z1', actor: 'admin' });
    expect(await zones.findById('z1')).toBeNull();
  });

  it('lève ZoneNotFoundError si absente', async () => {
    const uc = new DeleteZoneUseCase(new InMemoryZoneRepository(), new InMemoryCropZoneSuitabilityRepository(), new InMemoryZonePestPresenceRepository(), audit() as any, clock);
