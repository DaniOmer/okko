...
export function OperationEditor({ campaignId, initial, trigger, parcelGps }: { campaignId: string; initial?: OperationLog; trigger: React.ReactNode; parcelGps?: { lat?: number; lng?: number } }) {
  ...
  const fresh = (): OperationFormValue => ({ ...emptyOperation(), lat: parcelGps?.lat != null ? String(parcelGps.lat) : '', lng: parcelGps?.lng != null ? String(parcelGps.lng) : '' });
  ...
  async function submit() {
    ...
    try {
      ...
      setOpen(false); if (!initial) setForm(fresh()); router.refresh();
